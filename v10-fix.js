/* JLG Project Lab 360 — V10.6.1 shell stabilization */
(()=>{
  ...
  function branding(){
    document.title='JLG Project Lab 360 — V10.6.1 Autonome';
    document.documentElement.dataset.projectLabVersion=VERSION;
    const v=$('.version-pill');if(v)v.textContent='V10.6.1';
    const small=$('.brand small');if(small)small.textContent='V10.6.1 Autonome · sans compte, sans backend';
    const eye=$('.logo-stage .eyebrow');if(eye&&eye.textContent!=='JLG PROJECT LAB 360 · V10.6.1')eye.textContent='JLG PROJECT LAB 360 · V10.6.1';
    document.querySelectorAll('.brand-logo,.hero-logo,.splash-card img,.logo-stage-media img,.brand-ribbon img,.dossier-brand img').forEach(img=>{
      if(img.getAttribute('src')!==SVG)img.setAttribute('src',SVG);
      img.removeAttribute('srcset');img.style.objectFit='contain';img.style.objectPosition='center';img.style.display='block';
    });
  ...
  branding();unlockUi();
  setTimeout(()=>{branding();unlockUi()},300);
  setTimeout(()=>{branding();unlockUi()},1100);
