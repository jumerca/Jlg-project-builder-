/* JLG Project Lab 360 — V10.7 Android APK installer */
(()=>{
  ...
  function androidHelp(){
    ...
    m.innerHTML='<div style="width:min(460px,100%);background:#0a1724;color:white;border:1px solid #34506a;border-radius:20px;padding:20px"><h3>Installer JLG Project Lab 360</h3><p>L’APK Android va être téléchargé. Ouvre ensuite le fichier téléchargé et valide <b>Installer</b>.</p><p>Au premier essai, Android peut demander d’autoriser l’installation d’applications provenant de ce navigateur. C’est normal : active l’autorisation puis reviens au fichier.</p><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px"><a class="btn primary" href="'+APK_URL+'" download="JLG-Project-Lab-360.apk">Télécharger l’APK</a><button class="btn ghost" data-close type="button">Fermer</button></div></div>';
  ...
  async function install(){
    ...
    if(installPrompt){
      try{installPrompt.prompt();await installPrompt.userChoice;installPrompt=null;window.__jlgInstallPrompt=null;return}catch(e){console.warn(e)}
    }
